...
draft: false
description: "In the previous blog posts a basic setup for a RPI distribution with Yocto and a way to handle multiple layers synchronized with repo manifests were already discussed. This article is a step forward, making use of the knownledge gathered so far, yet allowing for even more automatization, all thanks to the Docker Containers."
---
...
Okay, but first let me walk you through a Docker install. For an extensive description checkout the https://docs.docker.com/engine/install/[official Docker documentation], but below should be just fine. 
...
# confirm that the docker is installed by running the hello world image
...
To get the docker group rights to take effect log out and back in or restart the machine.
